...
import { QueryWithConnection } from '../query';
...
describe('afterware', () => {
  it('pagination', async () => {
    ...
    expect(
      await query.resolve({}, { after: '10' }),
    ).toEqual(_.range(10, 1010).map(id => ({ id: `${id}` })));
    expect(
      await query.resolve({}, { after: '120', first: 100 }),
    ).toEqual(_.range(120, 220).map(id => ({ id: `${id}` })));

    resolve.mockImplementation(() => _.range(1, 2000).map(cursor => ({ cursor: `${cursor}` })));
    expect(
      await query.resolve({}, { after: '120', first: 100 }),
    ).toEqual(_.range(120, 220).map(cursor => ({ cursor: `${cursor}` })));
  });
});
